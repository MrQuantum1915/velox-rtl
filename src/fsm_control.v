...
module fsm_control #(
    ...
    // Control outputs
    output reg  compute_en,    // 1-cycle pulse
    output reg  display_en,    // enable 7-segment display
    output reg  timeout_err    // timeout indicator
);
    ...
    always @(posedge clk) begin
        if (!rst_n) begin
            ...
            case (state)
                ...
                S_WAIT_B: begin
                    timeout_cnt <= timed_out ? timeout_cnt : timeout_cnt + 1;
                    display_en  <= 0;
                end

                S_COMPUTE: begin
                    compute_en  <= 1;   // 1-cycle pulse
                    display_en  <= 0;
                    timeout_cnt <= 0;
                end
                ...
                S_DISPLAY: begin
                    display_en <= 1;
                    hold_cnt   <= hold_done ? hold_cnt : hold_cnt + 1;
                end
