...
module tb_top;
    ...
    // =========================================================================
    reg speed_prev, timeout_prev, ovf_prev;

always @(posedge CLK100MHZ) begin
    // store previous values
    speed_prev   <= dut.speed_valid;
    timeout_prev <= dut.LED_TIMEOUT;
    ovf_prev     <= dut.LED_OVERFLOW;

    // print ONLY on rising edge

    if (dut.speed_valid && !speed_prev)
        $display("[%0t ns] <<< speed_valid: speed_cms = %0d cm/s  too_fast=%b",
                 $time, dut.speed_cms, dut.too_fast);

    if (dut.LED_TIMEOUT && !timeout_prev)
        $display("[%0t ns] !!! TIMEOUT – object did not reach sensor B", $time);

    if (dut.LED_OVERFLOW && !ovf_prev)
